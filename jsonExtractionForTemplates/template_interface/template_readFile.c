...
Module:   ReadTemplateFile

Purpose:  This is called from 2 places
- InitTemplate is called once by the equipment class (not instance) by passing
a big template file (consists of all the templates within it)
- InitializeTemplate is called by each instance by passing individual templates

Filename: template_readFile.c
...
//tbool check if template already in cached then add only new one
ERROR_STATUS ReadTemplateFile(TCHAR* pTemplateName, SIGNED8** buffer,UNSIGNED8 tbool)
{
  ...
  if (pTemplateName == NULL)
  {
    //Read the Template Name from the resource.
	  if(tbool == FALSE)
	  {
		oreGetMyResource(RID_DATA_MODEL_TEMPLATE, &TemplateParm);
		pTemplateName = (TCHAR*)TemplateParm.parmValue.tString.strPtr;
	  }
	  else
	  {
		  oreGetMyResource(RID_DATA_MODEL_TEMPLATE_NEW, &TemplateParm);
			pTemplateName = (TCHAR*)TemplateParm.parmValue.tString.strPtr;
	  }
  }
  ...
  {
    status = loadDataModelFile(jsonFilePath, &fileBuffer);
  }
...
/*------------------------------------------------------------------------------
Module:   loadDataModelFile method
